...
/*   Updated: 2024/09/08 22:01:45 by mmuhaise         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	// printf("Handling special tokens at index %d\n", i);
...
void merge_adjacent_tokens(t_elem **tokens_ll)
{
    ...
    while (cur && cur->next)
    {
        if (//!cur->space_after && // No space after the current token
            (cur->token[strlen(cur->token) - 1] == '"' || cur->token[strlen(cur->token) - 1] == '\'') &&
...
int tokenize_input(char *input, t_elem **tokens_ll, t_my_env *my_env)
{
    ...
    return 1;
    //if (is_heredoc_context(tokens_ll))
