...
long long	ft_atoll(const char *str, int *valid)
{
    ...
    *valid = 1; // Assume valid by default
    ...
    while (*str && *str >= '0' && *str <= '9')
    {
        ...
        {
            *valid = 0; // Overflow detected
            return (0);
        }
        res = res * 10 + (*str - '0');
        str++;
    }
    if (*str != '\0') // Invalid characters in the string
        *valid = 0;
    return (res * sign);
}
...
int	execute_exit(t_ast_node *node, t_my_env *my_env)
{
    ...
    if (node->arr[1] && !is_numeric_argument(node->arr[1]))
    {
        ...
        ft_putendl_fd(": numeric argument required", 2);
        exit(2); // Exit with code 2 for non-numeric arguments
    }
    ...
    else
    {
        if (node->arr[1])
        {
            ...
            if (!valid || exit_code < 0 || exit_code > 255)
            {
                ...
                ft_putstr_fd(node->arr[1], 2);
                ft_putendl_fd(": numeric argument required", 2);
                exit(2); // Treat out-of-range or invalid numbers as error
            }
        }
        else
        ...
        exit(exit_code % 256); // Exit codes wrap around modulo 256
    }
    return (1);
}
