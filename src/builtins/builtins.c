...
int handle_cd_execution(char *arg, t_my_env *my_env)
{
    ...
    if (ft_strcmp(arg, "-") == 0)
    {
        ...
        ft_printf("%s\n", arg); // Print target directory when using `cd -`
    }
    ...
    if (change_directory(arg, my_env) == 0)
    {
        ...
        if (!pwd)
        {
            ...
            pwd = get_env_var("$PWD1", my_env); // Use fallback $PWD
            if (pwd)
            {
                char *new_pwd = ft_strjoin(pwd, "/..");

                // Update OLDPWD with the old value of PWD
                update_env_oldpwd(old, my_env);

                // Update PWD with the new fallback path
...
int execute_cd(char *arg, t_my_env *my_env)
{
    ...
    if (arg == NULL)
    {
        ...
        char *old = get_env_var("$PWD", my_env);
        update_env_oldpwd(old, my_env);
    ...
    // Delegate to handle_cd_execution for other cases
