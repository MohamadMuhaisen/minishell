...
char	*get_command_path(t_ast_node *node, t_my_env *my_env, int *exit_status)
{
	char	*command_path;
			...
			t_my_env *my_env, int *exit_status)
{
	...
	command_path = get_command_path(node, my_env, exit_status);
	handle_heredoc(node);
	handle_redirections_exec(node);
	execute_command(command_path, node, my_env, exit_status);
}
